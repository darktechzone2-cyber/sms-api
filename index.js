...
// Helper: extract the first cookie from a 'set-cookie' array
function getCookieFromHeaders(setCookieArray) {
    if (!setCookieArray || !setCookieArray.length) return null;
    // Take the first cookie header (usually the session)
...
async function login() {
    ...
    // We'll manage cookies manually
    let currentCookie = null;

    // 1. Get login page (capture any cookie)
    ...
    const r2 = await axios.post(`${BASE_URL}/signin`, form, {
        headers: loginHeaders,
        maxRedirects: 0,
        validateStatus: () => true
    });
    console.log("[LOGIN] POST status:", r2.status);

    // After login, the cookie may be updated
    const updatedCookie = getCookieFromHeaders(r2.headers['set-cookie']);
    if (updatedCookie) currentCookie = updatedCookie;
    if (!currentCookie) throw new Error("No session cookie received after login");
...
async function fetchNumbers() {
    ...
    // Extract raw phone numbers (index 3 of each row)
    const rawNumbers = (data.aaData || []).map(row => row[3]).filter(p => p);
    return rawNumbers;
}

// ---------- Country detection & number cleaning ----------
const COUNTRY_MAP = {
...
// Perform login at startup; if it fails, exit with error
login().catch(err => {
