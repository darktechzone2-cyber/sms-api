...
// ========== PANEL LOGIN & NUMBER FETCH (manual cookie handling) ==========
async function loginAndFetchNumbers() {
  let cookie = '';

  // 1. Get login page to extract captcha and capture cookies
  ...
  // 2. Submit login
  const loginResp = await axios.post(`${BASE_URL}/signin`,
    new URLSearchParams({ username: ST_CREDENTIALS.username, password: ST_CREDENTIALS.password, capt: ans }).toString(),
    {
      headers: {
  ...
  if (loginResp.headers['set-cookie']) {
    ...
    cookie = [...new Set([...cookie.split('; '), ...newCookies])].join('; ');
  }
...
async function fullSync() {
  ...
  try {
    ...
    console.log(`[SYNC] Inserted ${added} new numbers. Total: ${cleanPhones.length}`);
  } catch (err) {
...
app.get('/api/number', (req, res) => {
  ...
  db.all(sql, params, (err, rows) => {
    ...
    if (!numbers.length) return res.status(404).json({ success: false, error: 'No numbers available' });
